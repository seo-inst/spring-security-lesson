...
/**
    CustomMemberDetails : Spring Security의 UserDetails 구현체
                          why?  Spring Security가 이해할 수 있는 회원(사용자) 정보 형식
                                  예)  Servlet Interface 를  구현한 웹프로그램은
                                         Web Container가   service ()  라는 단일한 방식으로 실행한다
...
public class CustomMemberDetails implements UserDetails {
    ...
    // Member 엔티티를 감싸서 Spring Security가 이해할 수 있게 변환
    ...
    public String getUsername() {
        return member.getUsername();  // username으로 로그인
    }
